...
class MD_Learning_settings
{
	public static function settings_init(){
		// add_action( 'admin_menu', array( 'MD_Learning_settings', 'settings_options' ) );
		// add_action( 'admin_init', array( 'MD_Learning_settings', 'settings_init_call' ) );

	add_action( 'admin_menu', array( 'MD_Learning_settings','wppv_api_add_admin_menu' ) );

	add_action( 'admin_init', array( 'MD_Learning_settings','wppv_api_settings_init' ) );
	...
	public static function wppv_api_add_admin_menu(  ) {
	    add_options_page( 'Wp Post Views Settings', __('Wp Post Views Settings', 'wppv'), 'manage_options', 'settings-api-page', array( 'MD_Learning_settings','wppv_api_options_page' ) );
	}
	...
	public static function wppv_api_settings_init(  ) {
	    ...
	    add_settings_section(
	        ...
	        __( 'Settings for WP Post Views', 'wppv' ),array( 'MD_Learning_settings','wppv_api_settings_section_callback'),
	        'wppvPlugin'
	    );

	    add_settings_field(
	        'wppv_api_text_field_0',
	        __( 'Show post views coloumn', 'wppv' ),array( 'MD_Learning_settings','wppv_show_views_callback'),
	        'wppvPlugin',
	        'wppv_api_wppvPlugin_section'
	    );
	    add_settings_field(
	        'mdl_api_text_field_1',
	        __( 'Views filter on IP (If checked multiple views will not count From same IP)', 'wppv' ),array( 'MD_Learning_settings','filter_on_ip_callback'),
	        'wppvPlugin',
	        'mdl_api_wppvPlugin_section'
		);
		add_settings_field(
	        'mdl_api_post_checkbox_1',
	        __( 'Select your custom post type', 'wppv' ),array( 'MD_Learning_settings','select_post_type_callback'),
	        'wppvPlugin',
	        'mdl_api_wppvPlugin_section'
	    );
	}
	...
	public static function wppv_show_views_callback(  ) {
	    $options = get_option( 'mdl_api_settings' );
	    $checkbox_val = empty($options['mdl_api_text_field_0']) ? '' : $options['mdl_api_text_field_0'] ;
	    ?>
	    <input type='checkbox' name='mdl_api_settings[mdl_api_text_field_0]' value="1" <?php checked( 1, $checkbox_val, true ); ?>>
	    <?php
	}

	public static function filter_on_ip_callback(  ) {
	    $options = get_option( 'mdl_api_settings' );
	    $checkbox_val = empty($options['mdl_api_text_field_1']) ? '' : $options['mdl_api_text_field_1'] ;
	    ?>
	    <input type='checkbox' name='wppv_api_settings[mdl_api_text_field_1]' value="1" <?php checked( 1, $checkbox_val, true ); ?>>
	    <?php
	}
